...
export default function ActivityForm({ open, onClose, onSubmit, activity, buttonType }) {
  ...
  const activities = CORE_ACTIVITY;
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSubmit({ id: activity?.id, date, type, numberOfClients });
    setDate(getLocalDate());
    setType('')
    setNumberOfClients(null)
  };
    ...
    <Modal open={open} onClose={handleClose}>
      ...
      >
        <Typography variant="h5" component="h2" gutterBottom>
          {buttonType} Core Activity Entry
        </Typography>
